<x-app-layout>
    ...
    <!-- Back Button -->
    <a href="{{ route('web.channels.show', $channel->id) }}" class="back-button">
        <svg style="width: 20px; height: 20px;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
        </svg>
        {{ org_trans('back_to_channel') }}
    </a>
    ...
    <div class="edit-header">
        <h1>
            ...
            {{ org_trans('edit_channel') }}
        </h1>
        <p>{{ org_trans('edit_channel_description') }} "{{ $channel->name }}"</p>
    </div>
    ...
    <div class="form-card">
        ...
        <form action="{{ route('web.channels.update', $channel->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" class="form-input" value="{{ old('name', $channel->name) }}" required>
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" name="description" class="form-textarea">{{ old('description', $channel->description) }}</textarea>
            ...
            <div class="form-section">
                ...
                <div class="radio-group">
                    <div class="radio-option">
                        <input type="radio" id="type_public" name="type" value="public" {{ old('type', $channel->type) === 'public' ? 'checked' : '' }}>
                        <label for="type_public" class="radio-label">
                            ...
                            <div class="radio-label-desc">{{ org_trans('visible_to_all_can_join') }}</div>
                        </label>
                    </div>

                    <div class="radio-option">
                        <input type="radio" id="type_private" name="type" value="private" {{ old('type', $channel->type) === 'private' ? 'checked' : '' }}>
                        <label for="type_private" class="radio-label">
                            ...
                            <div class="radio-label-desc">{{ org_trans('accessible_by_invitation_only') }}</div>
                        </label>
                    </div>

                    <div class="radio-option">
                        <input type="radio" id="type_department" name="type" value="department" {{ old('type', $channel->type) === 'department' ? 'checked' : '' }}>
                        <label for="type_department" class="radio-label">
                            ...
                            <div class="radio-label-desc">{{ org_trans('reserved_for_department_members') }}</div>
                        </label>
                    </div>
                </div>
            </div>
            ...
            <div class="form-section" id="departmentSection" style="display: {{ old('type', $channel->type) === 'department' ? 'block' : 'none' }};">
                <div class="form-group">
                    ...
                    <select id="department_id" name="department_id" class="form-select">
                        ...
                        @foreach(\App\Models\Department::where('organization_id', $channel->organization_id)->get() as $dept)
            ...
            <div class="form-section">
                ...
                @php
                    $currentMembers = $channel->users->pluck('id')->toArray();
                @endphp

                <div class="members-list">
                    @foreach(\App\Models\User::where('organization_id', $channel->organization_id)->where('role', '!=', 'super_admin')->get() as $user)
                        <div class="member-checkbox">
                            <input type="checkbox" id="user_{{ $user->id }}" name="user_ids[]" value="{{ $user->id }}"
                                {{ in_array($user->id, old('user_ids', $currentMembers)) ? 'checked' : '' }}>
                            <label for="user_{{ $user->id }}">{{ $user->name }}</label>
                        </div>
                    @endforeach
                </div>
                <div class="form-hint" style="margin-top: 8px;">{{ org_trans('select_channel_members_hint') }}</div>
            </div>
            ...
            <div class="form-actions">
                ...
                <a href="{{ route('web.channels.show', $channel->id) }}" class="btn btn-secondary">
                    {{ org_trans('cancel') }}
                </a>
        ...
        <!-- Delete Form (hidden) -->
        <form id="deleteForm" action="{{ route('web.channels.destroy', $channel->id) }}" method="POST" style="display: none;">
            @csrf
            @method('DELETE')
        </form>
    </div>
    ...
    <script>
        ...
        // Show/hide department section based on channel type
        ...
        function confirmDelete() {
            if (confirm('{{ org_trans('confirm_delete_channel') }}')) {
